...
void ThreeDModel::drawBoundingBox(Shader* myShader)
{
	if(octree!=NULL)
	{
		...
		//octree->drawAllBoxes(myShader); //draws all children too
	}
}
...
void ThreeDModel::translateBB(Vector3d vector){
	octree->translate(vector.x, vector.y, vector.z);
}

void ThreeDModel::rotateBB(Vector3d vector){
	octree->translate(vector.x, vector.y, vector.z);
}
...
void ThreeDModel::getMinMaxValues(double *min, double *max){
	octree->getMinValues(min);
	octree->getMaxValues(max);
}
