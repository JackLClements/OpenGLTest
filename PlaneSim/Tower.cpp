...
void Tower::loadShader(Shader * shader){
	myShader = shader;
}


void Tower::loadModel(){
	//ThreeDModel tempModel; //temporary model object
	model = new ThreeDModel();
	if(obj->loadModel("TestModels/tower.obj", *model))//returns true if the model is loaded, puts the model in the model parameter
		{
...
ThreeDModel* Tower::getModel(){
	return model;
}

void Tower::getInfoForCollisions(){
...
double Tower::getDepth(){
	//return (max[2] - min[2])/2;
	return originalPoints[7].z - originalPoints[0].z;
}

double* Tower::getCenter(){
	return centerPoint;
}

double** Tower::getFaceNormals(){
	return faceNormals;
}
void Tower::moveBoundingBox(glm::vec3 vector){
	model->translateBB(Vector3d(vector.x, vector.y, vector.z));
}

void Tower::rotate(glm::mat4 matrix){ //can be used to rotate object, will not be used in this isntance, but is theoretically possible
	for(int i = 0; i < 8; i++){
